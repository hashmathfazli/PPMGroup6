...
import {
  ...
  apiGetAssessments, apiSubmitAssessment, apiRequestInfo, apiInfoResponse,
...
function LeadDetailPanel({
  ...
  function addComm() {
    ...
    apiAddInteraction(lead!.id, { type: entry.type, summary: entry.summary, document: entry.document, document_data: entry.documentData })
      .catch(() => {});
  }
            ...
            <>
              ...
              {addingComm && (
                <div className="rounded-xl p-4 flex flex-col gap-3" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
                  ...
                  {newCommDoc ? (
                    <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ background: "var(--card)", border: "1px solid #F9A80030" }}>
                      ...
                      <span className="text-xs flex-1 truncate" style={{ color: "#F9A800" }}>{newCommDoc}</span>
...
function LeadsView({
  ...
  return (
    <>
    <LeadDetailPanel lead={detailLead} onClose={() => setDetailLead(null)} isSalesRep={isSalesRep} isSalesManager={isSalesManager} onQualifyLead={onQualifyLead} onSubmitForAssessment={onSubmitForAssessment} onContactLead={onContactLead} assessments={assessments} onInfoResponse={onInfoResponse} onUpdateDetails={onUpdateLeadDetails} />
...
function DealsView({
  ...
  return (
    <div className="flex flex-col gap-6">
      <div>
        <h2 className="text-2xl font-bold tracking-tight">{isSalesRep ? "My Deals" : "Won Deals"}</h2>
        <p className="text-sm mt-0.5" style={{ color: "var(--muted-foreground)" }}>
          ...
          >
            <div className="w-full max-w-md h-full flex flex-col shadow-2xl" style={{ background: "var(--card)", borderLeft: "1px solid var(--border)" }}>
              ...
              <div className="flex items-center justify-between px-6 py-4 shrink-0" style={{ borderTop: "1px solid var(--border)" }}>
                <button onClick={() => { setSelectedUser(null); setEditingUser(null); }} className="px-4 py-2 rounded-lg text-sm font-medium hover:bg-[#ffffff08]" style={{ color: "var(--muted-foreground)" }}>
                  Cancel
                </button>
                <button
                  onClick={() => {
...
function AssessmentsView({
  ...
  leads?: typeof LEADS;
  users?: UserRecord[];
  onSubmitAssessment: (id: number, notes: string, risk: "Low" | "Medium" | "High", docs: { name: string; data: string }[]) => void;
  ...
  function AssessmentRow({ a }: { a: AssessmentRecord }) {
    return (
      <div className="flex flex-col gap-2">
        ...
        {/* Document chips for Sales Manager */}
        {isManager && (() => {
            ...
            {(isTechLead || isFinance) && (a.status === "Pending" || a.status === "In Review" || a.status === "Info Required") && (
              requestingId === a.id ? (
                ...
                <div className="flex flex-col gap-3 mt-3 pt-3" style={{ borderTop: "1px solid var(--border)" }}>
                  ...
                  {isTechLead && (() => {
                    const inp = (label: string, key: string, ph: string, rows = 2) => (
                      <div key={key}>
                    ...
                    return (
                      <div className="flex flex-col gap-3">
                        ...
                        <div>
                          <p className="text-xs font-medium mb-1" style={{ color: "var(--muted-foreground)" }}>Technical Feasibility</p>
                          <select value={techForm.feasibility ?? ""} onChange={(e) => tf("feasibility", e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: "var(--muted)", border: "1px solid var(--border)", color: "var(--foreground)" }}>
                            <option value="">— Select —</option>
                        ...
                        {inp("Requirement Assessment", "requirementAssessment", "Review each requirement and state whether it is feasible…")}
                        {inp("Technology Assessment", "technologyAssessment", "Suitability of required technologies, frameworks, databases…")}
                        {inp("Platform Assessment", "platformAssessment", "Web, mobile, desktop, cloud or on-premises platform suitability…")}
                        {inp("Integration Assessment", "integrationAssessment", "Can existing systems/APIs/third-party services be integrated?")}
                        {inp("Security Assessment", "securityAssessment", "Evaluation of security requirements implementation…")}
                        {inp("Performance Assessment", "performanceAssessment", "System capacity, response time, user load capability…")}
                        {inp("Resource Requirements", "resourceRequirements", "Developers, testers, DevOps, specialists required…")}
                        {inp("Technical Risks", "technicalRisks", "Possible risks, likelihood, impact, mitigation…")}
                        {inp("Technical Constraints / Conditions", "technicalConstraints", "Limitations, dependencies or conditions to consider…")}
                        <div>
                          <p className="text-xs font-medium mb-1" style={{ color: "var(--muted-foreground)" }}>Technical Recommendation</p>
                          <select value={techForm.recommendation ?? ""} onChange={(e) => tf("recommendation", e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: "var(--muted)", border: "1px solid var(--border)", color: "var(--foreground)" }}>
                            <option value="">— Select —</option>
                        ...
                        {inp("Technical Comments", "technicalComments", "Explanations, assumptions, additional information…", 3)}
                      </div>
                    );
                  })()}
                  ...
                  {isFinance && (() => {
                    ...
                    const numInp = (label: string, key: string, ph: string) => (
                      <div key={key}>
                        <p className="text-xs font-medium mb-1" style={{ color: "var(--muted-foreground)" }}>{label}</p>
                        <input type="text" placeholder={ph} value={finForm[key] ?? ""} onChange={(e) => ff(key, e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: "var(--muted)", border: "1px solid var(--border)", color: "var(--foreground)" }} onFocus={(e) => (e.currentTarget.style.borderColor = "#a78bfa")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--border)")} />
                      </div>
                    );
                    return (
                      <div className="flex flex-col gap-3">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {numInp("Estimated Project Cost", "projectCost", "e.g. USD 20,000")}
                          {numInp("Estimated Resource Cost", "resourceCost", "e.g. USD 14,000")}
                          {numInp("Estimated Infrastructure Cost", "infraCost", "e.g. USD 3,000")}
                          {numInp("Other Estimated Costs", "otherCosts", "e.g. USD 1,000")}
                          {numInp("Expected Profit", "expectedProfit", "e.g. USD 5,000")}
                          {numInp("Profit Margin", "profitMargin", "e.g. 25%")}
                        </div>
                        {inp("Payment Terms Assessment", "paymentTermsAssessment", "Client's expected payment schedule or conditions…")}
                        {inp("Budget Assessment", "budgetAssessment", "Is the client's budget sufficient for the proposed project?")}
                        {inp("Financial Risks", "financialRisks", "Insufficient budget, payment delays, unexpected costs…")}
                        {inp("Financial Conditions", "financialConditions", "Conditions such as advance payment or scope revision…")}
                        <div>
                          <p className="text-xs font-medium mb-1" style={{ color: "var(--muted-foreground)" }}>Financial Recommendation</p>
                          <select value={finForm.recommendation ?? ""} onChange={(e) => ff("recommendation", e.target.value)} className="w-full px-3 py-2 rounded-lg text-sm outline-none" style={{ background: "var(--muted)", border: "1px solid var(--border)", color: "var(--foreground)" }}>
                            <option value="">— Select —</option>
                        ...
                        {inp("Financial Comments", "financialComments", "Explain the financial assessment and recommendation…", 3)}
                      </div>
                    );
                  })()}
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex gap-2 ml-auto">
                      ...
                      <button
                        onClick={() => {
                          const extraNotes = isTechLead
                            ? Object.entries(techForm).filter(([,v]) => v).map(([k,v]) => `${k}: ${v}`).join("\n\n")
                            : isFinance
                            ? Object.entries(finForm).filter(([,v]) => v).map(([k,v]) => `${k}: ${v}`).join("\n\n")
                            : "";
                          const fullNotes = [notes, extraNotes].filter(Boolean).join("\n\n---\n\n");
                          onSubmitAssessment(a.id, fullNotes, risk, docs);
                          cancelSubmit();
                        }}
                        disabled={!notes.trim()}
                ...
                <div className="flex gap-2 mt-2 flex-wrap">
                  <button
                    onClick={() => { setSubmittingId(a.id); setNotes(a.notes); setRisk(a.risk); setDocs([]); }}
                    className="px-4 py-2 rounded-lg text-sm font-semibold transition-opacity hover:opacity-80"
            ...
            <div className="flex flex-col gap-5">
              <div className="p-4 rounded-xl flex flex-col gap-4" style={{ background: "var(--background)", border: "1px solid var(--border)" }}>
                ...
                <div>
                  ...
                  <input type="url" value={form.website} onChange={(e) => set("website", e.target.value)} placeholder="https://company.com" className={inputCls} style={inputStyle} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--primary)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--border)")} />
                </div>
              </div>
              <div className="p-4 rounded-xl flex flex-col gap-4" style={{ background: "var(--background)", border: "1px solid var(--border)" }}>
                ...
                <div>
                  <label className={labelCls} style={labelStyle}>Email Address *</label>
                  <input required type="email" value={form.contactEmail} onChange={(e) => set("contactEmail", e.target.value)} placeholder="contact@company.com" className={inputCls} style={inputStyle} onFocus={(e) => (e.currentTarget.style.borderColor = "var(--primary)")} onBlur={(e) => (e.currentTarget.style.borderColor = "var(--border)")} />
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div>
            ...
            <div className="p-4 rounded-xl flex flex-col gap-3" style={{ background: "var(--background)", border: "1px solid var(--border)" }}>
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  { label: "Closed Deals", value: myDeals.length },
                ].map((s) => (
                  <div key={s.label} className="px-3 py-2.5 rounded-lg text-center" style={{ background: "var(--muted)", border: "1px solid var(--border)" }}>
                    <p className="text-lg font-bold">{s.value}</p>
                    <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>{s.label}</p>
...
export default function App() {
  ...
  function handleSubmitAssessment(id: number, notes: string, risk: "Low" | "Medium" | "High", docs: { name: string; data: string }[]) {
    const now = new Date();
    const dateStr = now.toLocaleDateString("en-US", { month: "short", day: "numeric" });
    setAssessments((prev) => prev.map((a) => a.id === id ? { ...a, status: "Submitted", notes, risk, date: dateStr, documents: docs } : a));
    ...
    <div className="flex h-screen overflow-hidden" style={{ background: "var(--background)" }}>
      ...
      <main className="flex-1 overflow-y-auto">
        ...
        <div className="px-8 py-8 max-w-6xl">
          ...
          {activeView === "users" && <UsersView users={users} onAddUser={() => setNewUserOpen(true)} onToggleStatus={handleToggleUserStatus} onUpdateUser={handleUpdateUser} currentUserEmail={DEMO_CREDENTIALS.find((c) => c.name === userName)?.email} />}
